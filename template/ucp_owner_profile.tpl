    ...
    {foreach from=$OPP_UCP_OWNER_PROFILE.fields item=PROFILE_FIELD}
      ...
      <div class="form-group row align-items-center opp-owner-profile-field" data-field-key="{$PROFILE_FIELD.key|escape}" data-field-type="{$PROFILE_FIELD.type|escape}">
        ...
        <div class="col-12 col-md-7">
          ...
          {else}
            <input type="{if $PROFILE_FIELD.key == 'contact_number'}tel{else}text{/if}" class="form-control" id="opp-owner-profile-{$PROFILE_FIELD.key|escape}" name="opp_owner_profile[{$PROFILE_FIELD.key|escape}][value_text]" value="{$PROFILE_FIELD.value_text|escape}" {if !empty($PROFILE_FIELD.max_length)}maxlength="{$PROFILE_FIELD.max_length|escape}"{/if}{if $PROFILE_FIELD.key == 'contact_number'} inputmode="tel" autocomplete="tel" placeholder="+421 917 123 456" data-invalid-phone-message="{'Please add a valid contact phone number in My Profile first.'|@translate}"{/if} />
          {/if}
        </div>
      </div>
    {/foreach}
